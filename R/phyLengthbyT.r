phyLengthbyT_ <- function(t_1, phy, datatype="abundance"){
  ...
  if(datatype=="abundance"){
    ...
    phytable<-phytable %>% mutate(branch.length.new=ifelse(tgroup=="Root",0,
...
#'
#' @export
phyLengthbyT<- function(Ts, phy, datatype="abundance"){
  if (!inherits(phy, "chaophytree"))
    stop("Non convenient data : only for chaophytree object")
  ...
  sapply(Ts,function(x) phyLengthbyT_(x, phy, datatype))
